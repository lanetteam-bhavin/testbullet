...
public class StorageClientFragment extends android.app.Fragment
{
	...
	public void performFileSearch()
	{
		...
		// Filter to only show results that can be "opened", such as a file (as
		// opposed to a list
	...
	public void onActivityResult(int requestCode, int resultCode, Intent resultData)
	{
		...
		// READ_REQUEST_CODE.
		// If the request code seen here doesn't match, it's the response to
		// some other intent,
		...
		if (requestCode == READ_REQUEST_CODE && resultCode == Activity.RESULT_OK)
		{
			...
			// intent
			// provided to this method as a parameter. Pull that uri using
			// "resultData.getData()"
			Uri uri = null;
			if (resultData != null)
			{
	...
	public void showImage(Uri uri)
	{
		...
		if (uri != null)
		{
			// Since the URI is to an image, create and show a DialogFragment to
			// display the
			// image to the user.
	...
	public void dumpImageMetaData(Uri uri)
	{
		...
		// return one row.
		// no need to filter, sort, or select fields, since we want all fields
		// for one
		// document.
		...
		try
		{
			...
			if (cursor != null && cursor.moveToFirst())
			{
				...
				// an int can't be
				// null in java, the behavior is implementation-specific, which
				// is just a fancy
				// term for "unpredictable". So as a rule, check if it's null
				// before assigning
	...
	private class ImageDialogFragment extends DialogFragment
	{
		...
		public Dialog onCreateDialog(Bundle savedInstanceState)
		{
			...
			// To optimize for the "lightbox" style layout. Since we're not
			// actually displaying a
			// title, remove the bar along the top of the fragment where a
			// dialog title would
			// normally go.
			...
			// Loading the image is going to require some sort of I/O, which
			// must occur off the UI
			// thread. Changing the ImageView to display the image must occur ON
			// the UI thread.
			// The easiest way to divide up this labor is with an AsyncTask. The
			// doInBackground
			// method will run in a separate thread, but onPostExecute will run
			// in the main
			// UI thread.
